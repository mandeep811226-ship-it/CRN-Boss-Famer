...
public class MainActivity extends Activity {
    ...
    private View buildBossCard(String[] p) {
        ...
        final String capVal = sp.getString(capKey, "0");
        TextView capChip = chip(capVal,
            Color.argb(40, 59, 158, 255),
            C_BLUE,
            Color.argb(100, 59, 158, 255));
        capChip.setTextSize(12);
        capChip.setPadding(dp(14), dp(6), dp(14), dp(6));
        capChip.setOnClickListener(v -> editBossCap(catLabel, name, capVal));
        r3.addView(capChip, lpWH(-2, -2));
        c.addView(r3);

        // timer row — only shown when a spawn/respawn timer was parsed from the game
        if (timer != null && !timer.isEmpty() && !timer.equals("0")) {
            LinearLayout timerRow = row(Gravity.CENTER_VERTICAL);
            timerRow.setPadding(0, dp(3), 0, dp(1));
            TextView timerIcon = txt("⏱ ", 9, false, C_MUTED);
            timerRow.addView(timerIcon);
            TextView timerTv = txt(formatTimer(timer), 9, true, C_AMBER);
        ...
        // quick preset buttons: 1b · 3b · 5b
        LinearLayout presets = row(Gravity.CENTER_VERTICAL);
        presets.setPadding(0, dp(7), 0, 0);
        String[] presetVals = {"1b", "3b", "5b"};
        for (String preset : presetVals) {
            TextView pb = chip(preset,
                Color.argb(25, 59, 158, 255),
                C_TEXT2,
                Color.argb(70, 59, 158, 255));
            pb.setTextSize(10);
            pb.setPadding(dp(11), dp(5), dp(11), dp(5));
            final boolean isActive = preset.equalsIgnoreCase(capVal);
            if (isActive) {
                pb = chip(preset,
                    Color.argb(60, 59, 158, 255),
                    C_BLUE,
                    Color.argb(160, 59, 158, 255));
                pb.setTextSize(10);
                pb.setPadding(dp(11), dp(5), dp(11), dp(5));
            }
            final String presetVal = preset;
            pb.setOnClickListener(v -> {
                sp.edit().putString(capKey, presetVal).apply();
                toast("Cap → " + presetVal);
                showMain();
            });
            LinearLayout.LayoutParams plp = lpWH(-2, -2);
            plp.setMargins(0, 0, dp(5), 0);
            presets.addView(pb, plp);
        }
        c.addView(presets);
...
/**
    ...
    /** Formats a raw timer value (seconds or hh:mm:ss string) into a readable label. */
    private String formatTimer(String raw) {
        if (raw == null || raw.isEmpty()) return "";
        // Try colon-formatted string first (e.g. "01:30:45" or "30:45")
        if (raw.contains(":")) return raw.trim();
        try {
            long secs = Long.parseLong(raw.replaceAll("[^0-9]", ""));
            if (secs <= 0) return "";
